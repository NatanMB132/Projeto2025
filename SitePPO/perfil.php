...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['atualizar'])) {
    ...
    if ($novo_nome !== '') {
        $stmt = $conn->prepare("UPDATE usuarios1 SET usuario = ? WHERE usuario = ?");
    ...
    if ($nova_senha !== '') {
        ...
        $stmt = $conn->prepare("UPDATE usuarios1 SET senha = ? WHERE usuario = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['excluir'])) {
    $usuario = $_SESSION['usuario'];
    $stmt = $conn->prepare("DELETE FROM usuarios1 WHERE usuario = ?");
